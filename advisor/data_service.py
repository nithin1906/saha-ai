...
class StockDataService:
    ...
    def __init__(self):
        # Finnhub API key for fallback (get from environment variable)
        self.finnhub_key = os.environ.get('FINNHUB_API_KEY')
        # Cache timeout - 60 seconds for live data
        self.cache_timeout = 60
        
        # Log API key status (without exposing keys)
        logger.info(f"Finnhub API: {'Configured' if self.finnhub_key else 'Not configured (optional fallback)'}")
    ...
    def get_stock_price(self, symbol):
        ...
        if price:
            ...
            logger.info(f"Yahoo Finance (primary) success for {clean_symbol}: {price}")
            return price
        
        # FALLBACK: Finnhub API (requires API key)
        if self.finnhub_key:
            price = self._fetch_finnhub_api(clean_symbol)
            if price:
                cache.set(cache_key, price, self.cache_timeout)
                logger.info(f"Finnhub (fallback) success for {clean_symbol}: {price}")
                return price
    ...
    def _fetch_finnhub_api(self, symbol):
        """Fetch from Finnhub API (fallback for stocks)"""
        try:
            # For Indian stocks, Finnhub uses BSE/NSE format
            # Try NSE first, then BSE
            symbol_variants = [
                f"BSE:{symbol}",  # BSE format
                f"NSE:{symbol}",  # NSE format
                ...
                if response.status_code == 200:
                    ...
                    if price and price > 0:
                        ...
                        if self._validate_price(price_float, symbol):
                            logger.info(f"Finnhub success for {sym}: {price_float}")
                            return price_float
    ...
    def _fetch_yahoo_finance_api(self, symbol):
        """Fetch from Yahoo Finance API - PRIMARY METHOD (no API key required)"""
        try:
            import urllib3
                            ...
                            if '/chart/' in endpoint:
                                ...
                                if result and len(result) > 0:
                                    ...
                                    if price:
                                        price_float = float(price)
                                        if self._validate_price(price_float, symbol):
                                            logger.info(f"Yahoo Finance (chart API) success for {sym}: {price_float}")
                                            return price_float
                            else:
                                # Handle quote endpoint response
                                ...
                                if result:
                                    ...
                                    if price:
                                        price_float = float(price)
                                        if self._validate_price(price_float, symbol):
                                            logger.info(f"Yahoo Finance (quote API) success for {sym}: {price_float}")
                                            return price_float
                        
                    except Exception as e:
                        logger.debug(f"Yahoo Finance endpoint {endpoint} failed for {sym}: {e}")
                        continue
    ...
    def get_stock_history(self, symbol, period='1y', interval='1d'):
        ...
        """
        # Map frontend period to yfinance compatible period
        period_mapping = {
            '1d': '1d',
        ...
        yf_period = period_mapping.get(period.lower(), period) # fallback to original period
                    ...
                    if response.status_code == 200:
                        ...
                        if result and len(result) > 0:
                            ...
                            if timestamps and closes:
                                # Convert timestamps to dates and pair with prices
                                history = []
                                ...
                                if history:
                                    logger.info(f"Yahoo Finance history: Got {len(history)} data points for {sym}")
                                    return history
                
                except Exception as e:
                    logger.debug(f"Yahoo Finance history error for {sym}: {e}")
                    continue
            
            # Fallback to Finnhub if available
            if self.finnhub_key:
                return self._fetch_finnhub_history(symbol, yf_period)
    ...
    def _fetch_finnhub_history(self, symbol, period='1y'):
        """Fetch historical data from Finnhub (fallback)"""
        try:
            # Convert period to days for Finnhub
            period_days = {
            ...
            symbol_variants = [f"BSE:{symbol}", f"NSE:{symbol}", symbol]
            ...
            for sym in symbol_variants:
                ...
                if response.status_code == 200:
                    ...
                    if data.get('s') == 'ok' and data.get('c'):
                        ...
                        if history:
                            logger.info(f"Finnhub history: Got {len(history)} data points for {sym}")
                            return history
    ...
    def parse_intent(self, text):
        """
        Parse user intent from text to determine if it's a stock or MF analysis.
        """
        text = text.lower().strip()
        
        # Keywords for analysis
        analysis_keywords = ['analyze', 'analyse']
        stock_keywords = ['stock', 'share', 'equity']
        mf_keywords = ['mutual fund', 'mf', 'fund']

        # Check if the message is asking for analysis
        is_analysis_request = any(keyword in text for keyword in analysis_keywords)

        if is_analysis_request:
            # Remove analysis keywords to find the entity
            entity_text = text
            for keyword in analysis_keywords:
                entity_text = entity_text.replace(keyword, '')
            
            entity_text = entity_text.strip()

            # Remove stock/mf keywords
            for keyword in stock_keywords + mf_keywords:
                entity_text = entity_text.replace(keyword, '')
                
            entity_text = entity_text.replace(' a ', ' ').strip()
            entity_text = entity_text.replace(' an ', ' ').strip()
            
            # If entity is just 'a' or 'an' after stripping, it's empty
            if entity_text in ['a', 'an']:
                entity_text = ''

            # If after cleaning, the entity is empty, it's a request for a name
            if not entity_text:
                if any(kw in text for kw in stock_keywords):
                    return 'request_stock_name', 0.9, None
                if any(kw in text for kw in mf_keywords):
                    return 'request_mf_name', 0.9, None
                # If no specific keyword, but "analyze" was present, default to asking for a stock
                return 'request_stock_name', 0.8, None

            # We have an entity, now guess if it's a stock or MF
            if any(char.isdigit() for char in entity_text) or any(kw in text for kw in mf_keywords):
                 return 'analyze_mutual_fund', 0.8, entity_text
            else:
                return 'analyze_stock', 0.8, entity_text
